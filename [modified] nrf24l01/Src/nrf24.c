...
uint8_t nRF24_Check(void)
{
	...
	nRF24_WriteMBReg(nRF24_CMD_W_REGISTER | nRF24_REG_TX_ADDR, ptr, sizeof(nRF24_TEST_ADDR) - 1U);

	// Read it back to the buffer
	nRF24_ReadMBReg(nRF24_CMD_R_REGISTER | nRF24_REG_TX_ADDR, rxbuf, sizeof(nRF24_TEST_ADDR) - 1U);
...
void nRF24_SetAddr(uint8_t pipe, const uint8_t *addr)
{
	...
	switch (pipe)
	{
		...
		case nRF24_PIPE1:
			...
			nRF24_WriteMBReg(nRF24_CMD_W_REGISTER | nRF24_ADDR_REGS[pipe], (uint8_t*) addr, addr_width);
#else
...
void nRF24_TransmitPacket(uint8_t *pBuf, uint8_t length)
{
	...
	// *~*~*~*~*
	// Moved the status checking to a separate function which is called in the ISR.
	// *~*~*~*~*
...
// Function to check the nRF24's transmit status.
// This was originally in the TransmitPacket function however, that relied on polling method which
// blocked the CPU. This is a more efficient way to do this. The nRF24 generates an interrupt
// after the transmit. We check what flags are set in this function.
